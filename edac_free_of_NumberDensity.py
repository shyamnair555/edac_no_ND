...
def get_particle_array_edac(constants=None, **props):
    ...
    pa = get_particle_array(constants=constants, additional_props=EDAC_PROPS, **props)
    pa.set_output_arrays(['x', 'y', 'z', 'u', 'v', 'w', 'rho', 'p', 'au', 'av', 'aw', 'ap', 'm', 'h'])
    return pa

EDAC_SOLID_PROPS = ('ap', 'p0', 'wij', 'uf', 'vf', 'wf', 'ug', 'vg', 'wg', 'ax', 'ay', 'az', 'V')

def get_particle_array_edac_solid(constants=None, **props):
    "Get the fluid array for the transport velocity formulation"

    pa = get_particle_array(constants=constants, additional_props=EDAC_SOLID_PROPS, **props)
...
class EDACEquation(Equation):
    ...
    def loop(self, d_idx, d_m, d_rho, d_ap, d_p, d_V, s_idx, s_m, s_rho, s_p,
        ...
        fac = tmp * etaij * xdotdwij/(R2IJ + EPS)
        d_ap[d_idx] += fac * (d_p[d_idx] - s_p[s_idx])
...
class MomentumEquationPressureGradient(Equation):
    ...
    def initialize(self, d_idx, d_au, d_av, d_aw, d_auhat, d_pb0, d_avhat, d_awhat, d_p):
        d_au[d_idx] = 0.0
        d_av[d_idx] = 0.0
        d_aw[d_idx] = 0.0
        ...
        d_pb0[d_idx] = min(10*abs(d_p[d_idx]), self.pb)

    def loop(self, d_idx, s_idx, d_m, d_rho, s_rho, s_m,
             d_au, d_av, d_aw, d_p, d_pavg, d_pb0, XIJ, RIJ, SPH_KERNEL, HIJ, s_p,
             d_auhat, d_avhat, d_awhat, DWIJ):
        ...
        tmp = -d_pb0[d_idx] * s_m[s_idx]/rhoi2
...
class EDACScheme(Scheme):
    ...
    def _get_internal_flow_equations(self):
        from transport_velocity_free_of_NumberDensity import (
            SolidWallNoSlipBC, SummationDensity,
        ...
        for fluid in self.fluids:
            ...
            group2.extend([
                MomentumEquationArtificialStress(
                    dest=fluid, sources=fluids_with_io, dim=self.dim
                ),
                EDACEquation(
